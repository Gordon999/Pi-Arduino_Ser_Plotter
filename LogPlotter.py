#! /usr/bin/env python3
# Remark: this script needs the module tkcalendar
# install it with pip3 install tkcalendar before running
...
plot_length = 360
page_length = 300  #not yet relevant...
...
class Plot(Frame):
    def animate(self, i):
        if self.count > 0 and self.start == 1:
            self.ax1.clear()
            if self.legend == 1:
                plt.xlabel('Mid Time: ' + self.mid_time3 + ' Log: ' + self.log_file[len(log_path):] + " - LEFT for PREVIOUS, RIGHT for NEXT")
            elif self.legend == 0:
                plt.xlabel('Mid Time: ' + self.mid_time3 + ' Log: ' + self.log_file[len(log_path):] + " - RIGHT for NEXT")
            elif self.legend == 2:
                plt.xlabel('Mid Time: ' + self.mid_time3 + ' Log: ' + self.log_file[len(log_path):] + " - LEFT for PREVIOUS") 
            plt.ylabel('dB')
    ...
    def click(self,event):
        # RIGHT click (NEXT)
        if event.button == 3:
            self.mid_count += plot_length  - 1
            self.mid_time3 = str(datetime.datetime.strptime(self.mid_time3,'%H:%M:%S') + TimeDelta(seconds=plot_length ))[11:19]
            self.legend = 1
            if self.mid_count + int(plot_length /2) > self.count_x:
                self.mid_count = self.count_x - int(plot_length /2)
                self.legend = 2
            self.read_plot()
        # LEFT click (PREVIOUS)
        elif event.button == 1 :
            self.mid_count -= plot_length  -1
            self.legend = 1
            if self.mid_count < int(plot_length /2) + 1:
                self.mid_count = int(plot_length /2) + 1
                self.legend = 0
            self.read_plot()
    ...
    def Plot(self):
        ...
        self.mid_count = 0
        self.log_file = log_path + dat + ".log"
        self.read_plot()
    ...
    def read_plot(self):
        if os.path.exists (self.log_file):
            ...
            if self.count == 1:        
                ...
                start_count = self.mid_count - int(plot_length /2)
                end_count = self.mid_count + int(plot_length /2)
                # ensure not exceeding the ends of the log
                if start_count < 1:
                    start_count = 1
                    end_count = start_count + plot_length 
                elif end_count > self.count_x:
                    end_count = self.count_x
                    start_count = end_count - plot_length 
            ...
            if len(self.log) >= plot_length :
                for x in range(0,plot_length ):
                    if self.log[x][0:1] == "d":
